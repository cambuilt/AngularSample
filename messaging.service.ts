...
export class MessagingService implements OnDestroy, OnInit {
	...
	getMessageHistory() {
		const url = `https://${this.authService.subdomain === '' ? '' : this.authService.subdomain.replace('localhost:9876', 'rhodes') + '.'}emylabcollect.com/csp/rmp/${(this.authService.currentUser.role as String).toLowerCase().indexOf('tsa') > -1 ? 'tsa' : 'rsa'}/messaging/getMessageHistory?CSPCHD=${this.authService.sessionToken()}&CSPSHARE=1`;
